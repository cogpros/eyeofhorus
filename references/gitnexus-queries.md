...
## Graph Schema Quick Reference (v1.5.3)

**Nodes:** File, Function, Class, Interface, Method, Community, Process, Section, Folder, Route
**Edges (CodeRelation.type):** CALLS, IMPORTS, EXTENDS, IMPLEMENTS, DEFINES, MEMBER_OF, STEP_IN_PROCESS, CONTAINS, HAS_METHOD, HANDLES_ROUTE, FETCHES

Notes:
- Section and Folder are new in v1.5.3 and present across all repos. Section is the most numerous node type.
- CONTAINS is the dominant edge type (Folder->File, File->Section, etc).
- Route and HANDLES_ROUTE appear in repos with HTTP endpoints (your-app-repo, another-repo).
- FETCHES appears in one repo only.
- IMPLEMENTS and EXTENDS have zero instances in current indexes but are valid types (CodeRelation.type is a free string). They appear in repos with class inheritance/interface patterns.
